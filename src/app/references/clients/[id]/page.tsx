...
export default async function CreateClient({ id }) {
    ...
    if (!!id) {
        functionUtils.runIfNoFalse<IPatient>(await api.get<IPatient>(`clients/${id}`), (value) => { model = value })
    }
            ...
            <Stack spacing={2}>
                ...
                >
                    ...
                    <DatePickerElement name={"birthday"} label={"Дата рождения"}/>
                        <SelectElement name="sex" label={"Пол"} defaultValue={"other"} options={[
                    ...
                    <HAccordion label={"Контакты"}>
                        ...
                        <CheckboxElement name={"consentMailing"} label={"Согласие на рассылку"} />
                    </HAccordion>
                    ...
                    <HAccordion label={"Личные данные"}>
                        <SelectElement name="familyStatus" label={"Семейное положение"} defaultValue={"unknown"} options={[
                        ...
                        />
                        <SelectElement name="education" label={"Образование"} defaultValue={"unknown"} options={[
                            { id: "unknown", label: "Не известно"},
                            { id: "generalJunior", label: "Общее начальное"},
                            { id: "generalMiddle", label: "Общее среднее"},
                            { id: "generalSenior", label: "Общее полное"},
                            { id: "professionalMiddle", label: "Профессиональное: среднее"},
                            { id: "professionalSenior", label: "Профессиональное: высшее"}
                        ]}
                        />
                        <SelectElement name="employment" label={"Занятость"} defaultValue={"unknown"} options={[
                            { id: "unknown", label: "Не известно"},
                    ...
                    <HAccordion label={"Анамнез"}>
                        ...
                        <SelectElement name="bloodType" label={"Группа крови"} defaultValue={"unknown"} options={[
                            { id: "unknown", label: "Не известно"},
                        ...
                        <TextFieldElement name={'chronicDiseases'} label={"Хронические заболевания"} multiline/>
                    </HAccordion>
                </HEntityForm>
            </Stack>
        </Grid>
