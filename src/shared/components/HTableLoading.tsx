...
export interface HTableLoadingProps {
    columns: { title: string, align?: 'left' | 'right' | 'center', minWidth: number }[]
    pearPage: number
}
                        ...
                        <TableRow>
                            {
                                columns.map((column, index) => (
                                    <TableCell key={`title_${index.toString()}`} align={column.align} style={{ maxWidth: column.minWidth }}>{column.title}</TableCell>
                                ))
                            }
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {
                            new Array(pearPage).fill(null).map((_, index) => (
                                <TableRow hover role={"checkbox"} tabIndex={-1} key={`row_${index}`}>
                                    {
                                        columns.map((_, columnIndex) => <TableCell key={`column_${columnIndex}_${index}`}><Skeleton /></TableCell>)
                                    }
                                </TableRow>
                            ))
                        }
