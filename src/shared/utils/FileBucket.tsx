...
export class FileBucket {
  ...
  private async initBucket(): Promise<void> {
    const isExistBucket = await this.minioClient.bucketExists(
      FileBucket.BUCKET_NAME,
    );
    if (!isExistBucket)
      await this.minioClient.makeBucket(FileBucket.BUCKET_NAME);

    const bucketLifecycle = await this.minioClient
      .getBucketLifecycle(FileBucket.BUCKET_NAME)
      .then((value) => value)
      .catch(() => null);
    if (!bucketLifecycle)
      await this.minioClient.setBucketLifecycle(FileBucket.BUCKET_NAME, {
        Rule: [
          {
            ID: "Expiration Rule",
            Expiration: { Days: 1 },
  ...
  async uploadFile(file: File): Promise<string> {
    ...
    await this.minioClient.putObject(
      FileBucket.BUCKET_NAME,
      name,
      Readable.fromWeb(file.stream() as webStream.ReadableStream),
      file.size,
    );
